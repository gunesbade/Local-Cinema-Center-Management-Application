...
public class CashierDashboardController {
    ...
    public void finalizeReservation() {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            String insertTicket = "INSERT INTO Tickets (SessionID, CustomerName, SeatNumber, BasePrice, DiscountRate, FinalPrice) "
                    + "VALUES (?, ?, ?, ?, ?, ?)";
            PreparedStatement stmt = conn.prepareStatement(insertTicket);

            for (String seat : selectedSeats) {
                CustomerInfo ci = customerInfoMap.get(seat);
                if (ci == null) {
                    System.out.println("No customer info for seat " + seat);
                    continue;
                }
                BigDecimal disc = ci.getDiscountRate();
                BigDecimal discounted = basePrice.multiply(BigDecimal.ONE.subtract(disc));
                BigDecimal finalPrice = discounted.multiply(new BigDecimal("1.20"));

                String fullName = ci.getName() + " " + ci.getSurname();
                stmt.setInt(1, selectedSessionID);
                stmt.setString(2, fullName.trim());
                stmt.setString(3, seat);
                stmt.setBigDecimal(4, basePrice);
                stmt.setBigDecimal(5, disc);
                stmt.setBigDecimal(6, finalPrice);
                stmt.addBatch();

                totalTicketCost = totalTicketCost.add(finalPrice);
            }
            stmt.executeBatch();
            System.out.println("Ticket cost total: " + totalTicketCost);

            // Ürünler
            String insertSales = "INSERT INTO SalesProducts (ProductID, Quantity, FinalPrice) VALUES (?, ?, ?)";
            PreparedStatement spStmt = conn.prepareStatement(insertSales);

            BigDecimal totalProductCost = BigDecimal.ZERO;
            for (Product p : selectedProducts) {
                BigDecimal finalProd = p.getPrice().multiply(new BigDecimal("1.10"));
                totalProductCost = totalProductCost.add(finalProd);

                spStmt.setInt(1, p.getProductID());
                spStmt.setInt(2, 1);
                spStmt.setBigDecimal(3, finalProd);
                spStmt.addBatch();

                // stok düş
                String upSql = "UPDATE Products SET StockQuantity = StockQuantity - 1 WHERE ProductID=?";
                PreparedStatement ups = conn.prepareStatement(upSql);
                ups.setInt(1, p.getProductID());
                ups.executeUpdate();
            }
            spStmt.executeBatch();

            BigDecimal grand = totalTicketCost.add(totalProductCost);
            System.out.println("Product cost total: " + totalProductCost);
            System.out.println("Grand total: " + grand);

            cartMessageLabel.setText("Biletler ve ürünler kaydedildi! (KDV Dahil)");

            // *** Burada "işlem onaylanmıştır" mesajı ve "Ana ekrana dön" butonu ekliyoruz
            Alert alert = new Alert(AlertType.INFORMATION);
            alert.setTitle("Satış Onayı");
            alert.setHeaderText("İşlem onaylanmıştır!");
            alert.setContentText("Bilet ve ürün satın alımı başarıyla tamamlandı.");

            ButtonType mainScreenBtn = new ButtonType("Ana ekrana dön", ButtonBar.ButtonData.OK_DONE);
            alert.getButtonTypes().setAll(mainScreenBtn);

            Optional<ButtonType> result = alert.showAndWait();
            if (result.isPresent() && result.get() == mainScreenBtn) {
                // Ana ekrana dönmek için TabPane'in ilk sekmesine geçelim
                tabPane.getSelectionModel().select(0);
            }

        } catch (SQLException e) {
            e.printStackTrace();
            cartMessageLabel.setText("Hata oluştu.");
        }
    }
